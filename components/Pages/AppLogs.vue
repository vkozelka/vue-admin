<template>
  <admin-layout>
    <span slot="admin-page-title">{{ t('App Access Logs (Admin)') }}</span>

    <ui-grid ref="logsGrid" :component="this" :columns="columns" :filter="true" :pager="true" :limiter="true" :search="false" resource_url="backend/applicationlog/index" :limit_options="limitOpt" :export="true" :buttons="true" :buttons_options="buttons" :defaultOrderDir="'desc'">
      <template slot="grid-items" slot-scope="props">
        ...
        <td>
          <v-tooltip top>
            <div slot="activator" class="d-inline-block"><v-btn @click="blacklistIp(props.item.l.ip_source)" icon small><v-icon>cancel</v-icon></v-btn></div>
            <span v-text="'Add IP to Blacklist'"></span>
          </v-tooltip>
          <a target="_blank" :href="'http://en.utrace.de/?query='+ props.item.l.ip_source">{{ props.item.l.ip_source}}</a>
        </td>
    ...
    <hr />
    <h3>Exports</h3>
    <ui-grid ref="exportapplogGrid" :component="this" :filter="false"  :columns="exportColumns" :pager="false" :limiter="false" :search="false" resource_url="backend/applicationlog/export" :export="false">
      <template slot="grid-items" slot-scope="props">
