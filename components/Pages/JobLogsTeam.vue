<template>
  <admin-layout>
    <span slot="admin-page-title">{{ t('Job Access Logs (Team)') }}</span>
    <ui-grid ref="joblogsGrid" :component="this" :columns="columns" :filter="true" :pager="true" :limiter="true" :search="false" resource_url="backend/joblog/index" :export="false" :limit_options="limitOpt" :buttons="true" :buttons_options="buttons" :defaultOrderDir="'desc'">
      <template slot="grid-items" slot-scope="props">
        ...
        <td><a target="_blank" :href="'http://en.utrace.de/?query='+ props.item.jl.ip_source">{{ props.item.jl.ip_source}}</a></td>
