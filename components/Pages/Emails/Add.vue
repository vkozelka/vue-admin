<template>
  <admin-layout>
    <span slot="admin-page-title">{{t('Add email template')}}</span>
  ...
  export default {
    ...
    methods: {
      ...
      saveItem () {
        ...
        let item = this.item
        this.apiPost('backend/email/saveemail', item).then((response) => {
          if (response.body.error) {
            ...
            this.$router.push('/emails/' + response.body.item.id_email)
            this.$root.$emit('show-alert', {
              type: 'success',
              message: 'Email saved'
            })
