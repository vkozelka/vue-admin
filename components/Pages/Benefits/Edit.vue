  ...
  <admin-layout>
    <span slot="admin-page-title">{{t('Edit')}}: <span v-if="item">{{item.name}}</span></span>
        ...
        <v-toolbar>
          ...
          <v-btn color="red" flat @click="deleteItem(item.id_benefit)" :loading="delete_loader">
...
<script>
  export default {
    ...
    methods: {
      ...
      loadItem () {
        this.apiGet('backend/benefit/edit/' + this.$route.params.id).then((response) => {
      ...
      saveItem () {
        ...
        let item = this.item
        this.apiPost('backend/benefit/savebenefit/' + this.item.id_benefit, item).then((response) => {
          if (response.body.error) {
            this.$root.$emit('show-alert', {
              type: 'error',
              message: response.body.error
            ...
            this.languages.forEach((l) => {
              let p = new Promise((resolve, reject) => {
                this.apiPost('backend/benefit/savebenefitcontent/' + this.item.id_benefit, this.contents[l.id_language]).then((response) => {
                  if (response.body.error) {
