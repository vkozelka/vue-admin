<template>
  <admin-layout>
    <span slot="admin-page-title">{{t('New Benefit')}}</span>
  ...
  export default {
    ...
    methods: {
      ...
      saveItem () {
        ...
        let item = this.item
        this.apiPost('backend/benefit/savebenefit', item).then((response) => {
          if (response.body.error) {
            ...
            this.$router.replace('/benefits/' + response.body.item.id_benefit)
          }
        })
      },
      resetItem () {
